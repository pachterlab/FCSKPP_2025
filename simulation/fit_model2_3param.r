...
option_list <- list(
    make_option(c("-i", "--input_dir"), default="sim3_fit4"),
...
# message sim file location
log_msg(paste("Simulation files located in:", args$input_dir))
...
# Build 2x2 OU covariance matrix (general q)
...
# Precompute covariance structure for OU component (now takes q)
precompute_ou_cov_structure <- function(vcv_bm, alpha, beta, q, sigma_x1, sigma_x2,
    ...
    # Build coefficient matrix for theta means (using H with q)
    H <- matrix(c(alpha, 0, -q*beta, beta), 2, 2, byrow = TRUE)
    ...
    theta_pop_cov_2d_raw <- matrix(c(
        ...
        q * theta_b_pop_sd^2, theta_gamma_pop_sd^2 + q^2 * theta_b_pop_sd^2
    ), 2, 2)
...
precompute_noise_cov_structure <- function(N, theta_b_pop_sd, theta_gamma_pop_sd,
                                                 sigma_noise1, sigma_noise2, q) {
...
fast_single_trait_ou_ll <- function(y_vec, precomp_ou, theta_b_pop_mean, theta_gamma_pop_mean, q) {
    if (!precomp_ou$success) return(-1e10)
    
    theta_pop_mean_2d <- c(theta_b_pop_mean, theta_gamma_pop_mean  + q * theta_b_pop_mean)
...
fast_single_trait_noise_ll <- function(y_vec, precomp_noise, theta_b_pop_mean, theta_gamma_pop_mean, q) {
    if (!precomp_noise$success) return(-1e10)
    
    mu_noise <- rep(c(theta_b_pop_mean , theta_gamma_pop_mean + q*theta_b_pop_mean), precomp_noise$N)
...
# Fast mixture likelihood summing over all traits (now with q)
fast_mixture_likelihood_all_traits <- function(y_array, vcv_bm, alpha, beta, q, sigma_x1, sigma_x2,
    ...
    precomp_ou <- precompute_ou_cov_structure(vcv_bm, alpha, beta, q, sigma_x1, sigma_x2,
                                              theta_b_pop_sd, theta_gamma_pop_sd)
    precomp_noise <- precompute_noise_cov_structure(N, theta_b_pop_sd, theta_gamma_pop_sd,
                                                    sigma_noise1, sigma_noise2, q)
    ...
    for (k in 1:n_traits) {
        ...
        # Compute component likelihoods (pass q into the single-trait functions)
        ll_ou <- fast_single_trait_ou_ll(y_vec, precomp_ou, theta_b_pop_mean, theta_gamma_pop_mean, q)
        ll_noise <- fast_single_trait_noise_ll(y_vec, precomp_noise, theta_b_pop_mean, theta_gamma_pop_mean, q)
...
# NEW: Transform parameters from constrained to unconstrained space (added q)
# Order: log(alpha), log(beta), q, log(sigma_x1), log(sigma_x2),
#        log(sigma_noise1), log(sigma_noise2), qlogis(p_noise),
#        theta_mu_pop_mean, log(theta_mu_pop_sd), theta_gamma_pop_mean, log(theta_gamma_pop_sd)
params_to_unconstrained <- function(alpha, beta, q, sigma_x1, sigma_x2, sigma_noise1, sigma_noise2,
                                   p_noise, theta_b_pop_mean, theta_b_pop_sd,
                                   theta_gamma_pop_mean, theta_gamma_pop_sd) {
    c(log(alpha), log(beta), q,
      log(sigma_x1), log(sigma_x2), log(sigma_noise1), log(sigma_noise2),
      qlogis(p_noise),
...
unconstrained_to_params <- function(x) {
    list(
        ...
        p_noise = plogis(x[8]),
        theta_b_pop_mean = x[9],
        theta_b_pop_sd = exp(x[10]),
        theta_gamma_pop_mean = x[11],
        theta_gamma_pop_sd = exp(x[12])
    )
}
...
process_single_file_ml <- function(sim_file_path, optimizer, max_iter, reltol, n_starts) {
    ...
    max_rate <- min(100.0, 50.0 / tree_height)
    ...
    if (!is.null(sim_data$trait_theta_bs) && !is.null(sim_data$trait_theta_gammas)) {
        theta_b_init_mean <- mean(sim_data$trait_theta_bs, na.rm = TRUE)
        theta_b_init_var <- max(var(sim_data$trait_theta_bs, na.rm = TRUE), 0.01)
        theta_gamma_init_mean <- mean(sim_data$trait_theta_gammas, na.rm = TRUE)
        theta_gamma_init_var <- max(var(sim_data$trait_theta_gammas, na.rm = TRUE), 0.01)
    } else {
    ...
    objective <- function(params) {
        ...
        if (p$alpha > max_rate || p$beta > max_rate || 
            p$q*p$beta > max_rate || p$q*p$beta < -max_rate ||
            p$sigma_x1 > max_sigma || p$sigma_x2 > max_sigma ||
            p$sigma_noise1 > max_sigma || p$sigma_noise2 > max_sigma ||
            p$theta_b_pop_sd > max_sigma || p$theta_gamma_pop_sd > max_sigma ||
            abs(p$theta_b_pop_mean) > max_sigma*5 || abs(p$theta_gamma_pop_mean) > max_sigma*5 ||
            p$q>2.0 || p$q< -1.0) {
            return(1e10)
        }
    ...
    result <- list(
        ...
        standard_errors = list(
            ...
            q_se = if(!is.na(se_params[3])) se_params[3] else NA,
            ...
            theta_b_pop_mean_se = if(!is.na(se_params[9])) se_params[9] else NA,
            theta_b_pop_sd_se = if(!is.na(se_params[10])) sqrt(p$theta_b_pop_sd^2 * se_params[10]^2) else NA,
            theta_gamma_pop_mean_se = if(!is.na(se_params[11])) se_params[11] else NA,
            theta_gamma_pop_sd_se = if(!is.na(se_params[12])) sqrt(p$theta_gamma_pop_sd^2 * se_params[12]^2) else NA
        ),
        optimization = list(
        ...
        theta_init_info = list(theta_b_mean = theta_b_init_mean, theta_b_var = theta_b_init_var,
                               theta_gamma_mean = theta_gamma_init_mean, theta_gamma_var = theta_gamma_init_var),
        timing = list(optimization = NA),
        settings = list(mixture_model = "2D-OU-Gaussian-ComponentTheta-ML-NoME-q", n_parameters = 12, 
                       parameterization = "2d_component_theta_with_q", measurement_error = FALSE)
    )

    # Compute trait classification using precomputed structures
    precomp_ou <- precompute_ou_cov_structure(vcv_bm, p$alpha, p$beta, p$q, p$sigma_x1, p$sigma_x2,
                                              p$theta_b_pop_sd, p$theta_gamma_pop_sd)
    precomp_noise <- precompute_noise_cov_structure(n_tips, p$theta_b_pop_sd, p$theta_gamma_pop_sd,
                                                    p$sigma_noise1, p$sigma_noise2, p$q)
    ...
    for (k in 1:n_traits) {
        ...
        ll_ou <- suppressWarnings(fast_single_trait_ou_ll(y_vec, precomp_ou, 
                                                          p$theta_b_pop_mean, p$theta_gamma_pop_mean, p$q))
