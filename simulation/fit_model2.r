...
option_list <- list(
    make_option(c("-i", "--input_dir"), default="sim2_UB_40"),
...
## Change these based on the kind of input files (Simulated under model 2 vs model 1).
...
build_2d_ou_cov_matrix_general <- function(vcv_bm, alpha, beta, sigma_x1, sigma_x2) {
    ...
    N <- nrow(vcv_bm)
    # H-matrix changed from model 1.
    H <- matrix(c(alpha, 0, -beta, beta), 2, 2, byrow = TRUE)
    Sigma <- diag(c(sigma_x1, sigma_x2))
    M <- Sigma %*% t(Sigma)
    # Lyapunov function has been changed from model 1.
    ...
    for (i in 1:N) {
        ...
        if (i > 1) {
            for (j in 1:(i-1)) {
                ...
                exp_neg_H_tj <- exp_cache[[as.character(t_j)]]; if (is.null(exp_neg_H_tj)) exp_neg_H_tj <- expm(-H*t_j) # matrix_exp_2x2(-H, t_j)
                exp_neg_H_tmrca <- exp_cache[[as.character(t_mrca)]]; if (is.null(exp_neg_H_tmrca)) exp_neg_H_tmrca <- expm(-H*t_mrca) # matrix_exp_2x2(-H, t_mrca)
                ...
                exp_neg_H_dt_i <- expm(-H*dt_i) # matrix_exp_2x2(-H, dt_i)
                exp_neg_H_dt_j <- expm(-H*dt_j) # matrix_exp_2x2(-H, dt_j)
...
precompute_ou_cov_structure <- function(vcv_bm, alpha, beta, sigma_x1, sigma_x2,
    ...
    # Again, H is changed from model 1.
    H <- matrix(c(alpha, 0, -beta, beta), 2, 2, byrow = TRUE)
    mu_theta_coeff <- matrix(0, 2*N, 2)
    for (i in 1:N) {
        t_i <- vcv_bm[i,i]
        exp_neg_H_ti <- expm(-H*t_i)#matrix_exp_2x2(-H, t_i)
    ...
    # new X is given by theta_b,  theta_b - theta_mu
    theta_pop_cov_2d_raw <- matrix(c(
        theta_b_pop_sd^2, theta_b_pop_sd^2,
        theta_b_pop_sd^2, theta_b_pop_sd^2 + theta_mu_pop_sd^2
    ), 2, 2)
...
precompute_noise_cov_structure <- function(N, theta_mu_pop_sd, theta_b_pop_sd,
...
fast_single_trait_ou_ll <- function(y_vec, precomp_ou, theta_mu_pop_mean, theta_b_pop_mean) {
    if (!precomp_ou$success) return(-1e10)

    theta_pop_mean_2d <- c(theta_b_pop_mean, theta_b_pop_mean - theta_mu_pop_mean)
...
fast_single_trait_noise_ll <- function(y_vec, precomp_noise, theta_mu_pop_mean, theta_b_pop_mean) {
    if (!precomp_noise$success) return(-1e10)

    mu_noise <- rep(c(theta_b_pop_mean, theta_b_pop_mean - theta_mu_pop_mean), precomp_noise$N)
...
fast_mixture_likelihood_all_traits <- function(y_array, vcv_bm, alpha, beta, sigma_x1, sigma_x2,
    ...
    precomp_ou <- precompute_ou_cov_structure(vcv_bm, alpha, beta, sigma_x1, sigma_x2,
                                              theta_mu_pop_sd, theta_b_pop_sd)
    precomp_noise <- precompute_noise_cov_structure(N, theta_mu_pop_sd, theta_b_pop_sd,
                                                    sigma_noise1, sigma_noise2)
    ...
    for (k in 1:n_traits) {
        ...
        ll_ou <- fast_single_trait_ou_ll(y_vec, precomp_ou, theta_mu_pop_mean, theta_b_pop_mean)
        ll_noise <- fast_single_trait_noise_ll(y_vec, precomp_noise, theta_mu_pop_mean, theta_b_pop_mean)
...
unconstrained_to_params <- function(x) {
    list(
        ...
        theta_b_pop_mean = x[10],
        theta_b_pop_sd = exp(x[11])
    )
}
...
process_single_file_ml <- function(sim_file_path, optimizer, max_iter, reltol, n_starts) {
    ...
    if (!is.null(sim_data$trait_theta_mus) && !is.null(sim_data$trait_theta_bs)) {
    ...
    result <- list(
        ...
        standard_errors = list(
            ...
            theta_b_pop_mean_se = if(!is.na(se_params[10])) se_params[10] else NA,
            theta_b_pop_sd_se = if(!is.na(se_params[11])) sqrt(p$theta_b_pop_sd^2 * se_params[11]^2) else NA
        ),
        optimization = list(
        ...
        settings = list(mixture_model = "2D-OU-Gaussian-ComponentTheta-ML-NoME", n_parameters = 11, 
                       parameterization = "2d_component_theta", measurement_error = FALSE)
    )

    # Compute trait classification using precomputed structures
    precomp_ou <- precompute_ou_cov_structure(vcv_bm, p$alpha, p$beta, p$sigma_x1, p$sigma_x2,
                                              p$theta_mu_pop_sd, p$theta_b_pop_sd)
    precomp_noise <- precompute_noise_cov_structure(n_tips, p$theta_mu_pop_sd, p$theta_b_pop_sd,
    ...
    for (k in 1:n_traits) {
        ...
        ll_ou <- suppressWarnings(fast_single_trait_ou_ll(y_vec, precomp_ou, 
                                                          p$theta_mu_pop_mean, p$theta_b_pop_mean))
    ...
    output_file <- file.path(dirname(sim_file_path), paste0("fit_model2", tools::file_path_sans_ext(basename(sim_file_path)), ".rds"))
