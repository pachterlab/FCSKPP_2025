...
parse_cmd_args <- function() {
  option_list <- list(
    make_option(c("-o", "--output_dir"), type="character", default="sim1_fit1",
    ...
    make_option(c("--alpha_max"), type="numeric", default=2.0,
    ...
    make_option(c("--beta_max"), type="numeric", default=2.0,
...
simulate_2d_ou <- function(tree, alpha, beta, sigma_x1, sigma_x2, theta, x0) {
  ...
  cat('H matrix is:\n')
  print(modelOU$H)
  cat('Sigma_x matrix is:\n')
  print(modelOU$Sigma_x)  
  print(PCMTable(modelOU), xtable = TRUE, type="html")
...
simulate_2d_traits_mixture <- function(tree, n_traits, me_values, params) {
  ...
  for (i in 1:n_traits) {
    ...
    trait_theta <- c(trait_theta_mu + trait_theta_gamma, trait_theta_gamma)
