...
fast_single_trait_ou_ll <- function(y_vec, precomp_ou, theta_mu_pop_mean, theta_gamma_pop_mean) {
...
fast_single_trait_noise_ll <- function(y_vec, precomp_noise, theta_mu_pop_mean, theta_gamma_pop_mean) {
    if (!precomp_noise$success) return(-1e10)
    
    mu_noise <- rep(c(theta_mu_pop_mean + theta_gamma_pop_mean, theta_gamma_pop_mean), precomp_noise$N)
...
fast_mixture_likelihood_all_traits <- function(y_array, vcv_bm, alpha, beta, sigma_x1, sigma_x2,
    ...
    for (k in 1:n_traits) {
        ...
        ll_ou <- fast_single_trait_ou_ll(y_vec, precomp_ou, theta_mu_pop_mean, theta_gamma_pop_mean)
        ll_noise <- fast_single_trait_noise_ll(y_vec, precomp_noise, theta_mu_pop_mean, theta_gamma_pop_mean)
