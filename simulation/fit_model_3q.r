...
# Lyapunov solver for H = [[alpha, -q*alpha], [0, beta]]
lyapunov_ab <- function(alpha, beta, q, M) {
    # Solve H V + V H' = M for symmetric V = [[v11, v12],[v12,v22]]
    # where H = [[alpha, -q*alpha],[0, beta]]
    if (!is.matrix(M)) M <- as.matrix(M)
    v22 <- M[2,2] / (2 * beta)
    v12 <- (M[1,2] + q * alpha * v22) / (alpha + beta)
    v11 <- M[1,1] / (2 * alpha) + q * v12
...
build_2d_ou_cov_matrix_general <- function(vcv_bm, alpha, beta, q, sigma_x1, sigma_x2) {
    ...
    N <- nrow(vcv_bm)
    H <- matrix(c(alpha, -q * alpha, 0, beta), 2, 2, byrow = TRUE)
...
precompute_ou_cov_structure <- function(vcv_bm, alpha, beta, q, sigma_x1, sigma_x2,
    ...
    theta_pop_cov_2d_raw <- matrix(c(
        theta_mu_pop_sd^2 + q^2 * theta_gamma_pop_sd^2, q * theta_gamma_pop_sd^2,
        q * theta_gamma_pop_sd^2, theta_gamma_pop_sd^2
    ), 2, 2)
...
fast_single_trait_ou_ll <- function(y_vec, precomp_ou, theta_mu_pop_mean, theta_gamma_pop_mean, q) {
    if (!precomp_ou$success) return(-1e10)
    
    theta_pop_mean_2d <- c(theta_mu_pop_mean + q * theta_gamma_pop_mean, theta_gamma_pop_mean)
...
fast_single_trait_noise_ll <- function(y_vec, precomp_noise, theta_mu_pop_mean, theta_gamma_pop_mean, q) {
    if (!precomp_noise$success) return(-1e10)
    
    mu_noise <- rep(c(theta_mu_pop_mean + q * theta_gamma_pop_mean, theta_gamma_pop_mean), precomp_noise$N)
...
fast_mixture_likelihood_all_traits <- function(y_array, vcv_bm, alpha, beta, q, sigma_x1, sigma_x2,
    ...
    precomp_ou <- precompute_ou_cov_structure(vcv_bm, alpha, beta, q, sigma_x1, sigma_x2,
                                              theta_mu_pop_sd, theta_gamma_pop_sd)
    precomp_noise <- precompute_noise_cov_structure(N, theta_mu_pop_sd, theta_gamma_pop_sd,
                                                    sigma_noise1, sigma_noise2, q)
    ...
    for (k in 1:n_traits) {
        ...
        ll_ou <- fast_single_trait_ou_ll(y_vec, precomp_ou, theta_mu_pop_mean, theta_gamma_pop_mean, q)
        ll_noise <- fast_single_trait_noise_ll(y_vec, precomp_noise, theta_mu_pop_mean, theta_gamma_pop_mean, q)
...
process_single_file_ml <- function(sim_file_path, optimizer, max_iter, reltol, n_starts) {
    ...
    if (!is.null(sim_data$trait_theta_mus) && !is.null(sim_data$trait_theta_gammas)) {
        ...
        for (k in 1:n_traits) {
            ...
            # This assumes q=1, maybe okay as a starting point.
    ...
    objective <- function(params) {
        ...
        if (p$alpha > max_rate || p$beta > max_rate ||
            p$q*p$alpha > max_rate || p$q*p$alpha < -max_rate || 
    ...
    precomp_noise <- precompute_noise_cov_structure(n_tips, p$theta_mu_pop_sd, p$theta_gamma_pop_sd,
    ...
    for (k in 1:n_traits) {
        ...
        ll_ou <- suppressWarnings(fast_single_trait_ou_ll(y_vec, precomp_ou, 
                                                          p$theta_mu_pop_mean, p$theta_gamma_pop_mean, p$q))
