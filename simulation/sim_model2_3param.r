...
parse_cmd_args <- function() {
  option_list <- list(
    make_option(c("-o", "--output_dir"), type="character", default="sim4",
    ...
    make_option(c("--alpha_max"), type="numeric", default=20.0,
    ...
    make_option(c("--beta_max"), type="numeric", default=20.0,
                help="Maximum value for beta parameter"),
    make_option(c("--q_min"), type="numeric", default=-1.0,
                help="Minimum value for q parameter"),
    make_option(c("--q_max"), type="numeric", default=2.0,
                help="Maximum value for q parameter"),
    ...
    make_option(c("--theta_gamma_mean"), type="numeric", default=0,
                help="Mean of theta_gamma distribution for population sampling"),
    make_option(c("--theta_gamma_sd"), type="numeric", default=1.0,
                help="Standard deviation of theta_gamma distribution for population sampling"),
...
simulate_2d_ou <- function(tree, alpha, beta, sigma_x1, sigma_x2, theta, x0, q=0.0) {
  # Load PCMBase
  ...
  # H matrix now: [[alpha, -q*alpha], [0, beta]]
  modelOU$H[1,1] <- alpha
  modelOU$H[1,2] <- 0
  modelOU$H[2,1] <- -q*beta
  modelOU$H[2,2] <- beta   
  
  # Sigma_x (PCMBase uses sigma, not sigma^2)
  modelOU$Sigma_x[1] <- sigma_x1
  modelOU$Sigma_x[4] <- sigma_x2
  ...
  # Transpose/arrange to have 2 x n_tips matrix
...
simulate_2d_traits_mixture <- function(tree, n_traits, me_values, params) {
  ...
  for (i in 1:n_traits) {
    ...
    trait_theta_gamma <- rnorm(1, mean = params$theta_gamma_pop_mean, sd = params$theta_gamma_pop_sd)
    
    trait_theta_bs[i] <- trait_theta_b
    trait_theta_gammas[i] <- trait_theta_gamma
    
    # New optimum uses q: (theta_b , theta_gamma + q * theta_b)
    trait_theta <- c(trait_theta_b, trait_theta_gamma + params$q* trait_theta_b)
    ...
    if (is_noise[i]) {
      ...
      trait_data <- simulate_2d_ou(tree, params$alpha, params$beta, 
                                       ...
                                       trait_theta, trait_x0, q = params$q)
      trait_types[i] <- "phylo"
    }
    
    # Add measurement error (element-wise multiplied by standard normal draws)
  ...
  return(list(
    ...
    theta_gammas = trait_theta_gammas,
    thetas = trait_thetas,
    n_phylo = n_phylo, 
    n_noise = n_noise
  ))
...
generate_random_params <- function(args) {
  ...
  # q in [0,1], sampled uniformly per repeat
  params$q <- runif(1, args$q_min, args$q_max)

  params$theta_b_pop_mean <- args$theta_b_mean
  params$theta_b_pop_sd <- args$theta_b_sd
  params$theta_gamma_pop_mean <- args$theta_gamma_mean
  params$theta_gamma_pop_sd <- args$theta_gamma_sd
...
create_filename <- function(output_dir, params, repeat_num) {
  ...
  q_str <- paste0("Q", sprintf("%.3f", params$q))

  theta_str <- paste0("ThetaB_", sprintf("%.1f", params$theta_b_pop_sd), 
                     "_ThetaGam_", sprintf("%.1f", params$theta_gamma_pop_sd))
  
  filename <- file.path(output_dir, paste0("sim_2d_", repeat_num, "_", alpha_str, "_", beta_str, "_", 
                                          sigma1_str, "_", sigma2_str, "_", sn1_str, "_", sn2_str, "_", 
                                          p_str, "_", q_str, "_", theta_str, ".rds"))
...
main <- function() {
  ...
  if (!args$quiet) {
    cat("\nStarting 2D OU mixture model simulation (with per-repeat q in [0,1]):\n")
    ...
    cat("- q range: [", args$q_min, ",", args$q_max, "]\n")
    ...
    cat("- Theta_gamma ~ N(", args$theta_gamma_mean, ",", args$theta_gamma_sd, ") per trait\n\n")
  }
  ...
  for (repeat_num in 1:args$repeats) {
    ...
    if (args$verbose) {
      cat("Selection matrix parameters: alpha =", sprintf("%.3f", params$alpha), 
          "beta =", sprintf("%.3f", params$beta), "q =", sprintf("%.3f", params$q), "\n")
    ...
    result <- list(
      ...
      trait_theta_gammas = sim_result$theta_gammas,
      ...
      parameters = list(
        ...
        theta_mu_pop_mean = params$theta_b_pop_mean,
        theta_mu_pop_sd = params$theta_b_pop_sd,
        theta_gamma_pop_mean = params$theta_gamma_pop_mean,
        theta_gamma_pop_sd = params$theta_gamma_pop_sd
      ),
      n_traits = args$ntraits,
      n_phylo_traits = sim_result$n_phylo,
      n_noise_traits = sim_result$n_noise,
      tip_labels = tree$tip.label,
      model_type = "2D_OU_mixture_q"
    )
