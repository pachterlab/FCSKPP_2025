...
precompute_ou_cov_pieces <- function(vcv_bm, alpha, sigma_x,
...
precompute_noise_pieces <- function(N, theta_mu_pop_sd) {
...
fast_single_trait_ou_ll <- function(y_vec, pieces_ou, me_var_vec, theta_mu_pop_mean) {
...
fast_single_trait_noise_ll <- function(y_vec, pieces_noise, me_var_vec, sigma_noise1, sigma_noise2, theta_mu_pop_mean) {
    ...
    mu_noise <- rep(c(theta_mu_pop_mean , theta_mu_pop_mean), N)
...
fast_mixture_likelihood_all_traits <- function(y_array, vcv_bm, alpha,  sigma_x,
                                              ...
                                              me_array, epsilon) {
    # me_array: n_tips x n_traits x 2  (measurement std devs or measurement "sizes" — we square them to variances)
    N <- nrow(y_array)
    n_traits <- ncol(y_array)
    
    # Precompute OU & noise pieces once (trait invariant)
    pieces_ou <- precompute_ou_cov_pieces(vcv_bm, alpha, sigma_x,
                                         theta_mu_pop_sd, epsilon)
    pieces_noise <- precompute_noise_pieces(N, theta_mu_pop_sd)
    ...
    for (k in 1:n_traits) {
        ...
        ll_ou <- fast_single_trait_ou_ll(y_vec, pieces_ou, me_var_vec, theta_mu_pop_mean)
...
process_single_file_ml <- function(b_file, gamma_file, error_b_file, error_gamma_file, optimizer, max_iter, reltol, n_starts) {
    ...
    best_params <- as.numeric(best_result[1, 1:7])
    ...
    result <- list(
        ...
        optimization = list(
            ...
            bic = log(n_traits * n_tips * 2) * num_params - 2 * log_likelihood,
        ...
        settings = list(mixture_model = "2D-OU-Gaussian-ComponentTheta-ML-WithME", n_parameters = num_params, 
    ...
    pieces_ou_final <- precompute_ou_cov_pieces(vcv_bm, p$alpha,  p$sigma_x, 
                                              p$theta_mu_pop_sd, epsilon)
    pieces_noise_final <- precompute_noise_pieces(n_tips, p$theta_mu_pop_sd)
    ...
    if (is.null(args$expression_bin)) {
        output_file <- "data_results/data_fit_qsd_001.rds"
    }
    else {
        output_file <- paste0("data_results/data_fit_qsd_001_", args$expression_bin, '.rds')
    }
