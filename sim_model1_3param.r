...
parse_cmd_args <- function() {
  option_list <- list(
    make_option(c("-o", "--output_dir"), type="character", default="sim3",
    ...
    make_option(c("--q_min"), type="numeric", default=-1,
                help="Minimum value for q parameter"),
    make_option(c("--q_max"), type="numeric", default=2,
...
simulate_2d_ou <- function(tree, alpha, beta, sigma_x1, sigma_x2, theta, x0, q=0.0) {
  ...
  modelOU$H[1,1] <- alpha
  modelOU$H[1,2] <- -q * alpha
  modelOU$H[2,1] <- 0
  modelOU$H[2,2] <- beta   # check convention?
...
simulate_2d_traits_mixture <- function(tree, n_traits, me_values, params) {
  ...
  for (i in 1:n_traits) {
    ...
    # New optimum uses q: (theta_b + q*theta_gamma, theta_gamma)
    trait_theta <- c(trait_theta_mu + params$q * trait_theta_gamma, trait_theta_gamma)
